...
- Code, which contains a code preface (which could be imports, global
...
class Code:
  ...
  def find_function_index(self, function_name: str) -> int:
    ...
    if count == 0:
      raise ValueError(
          f'function {function_name} does not exist in code:\n{str(self)}'
      )
    if count > 1:
      raise ValueError(
          f'function {function_name} exists more than once in code:\n'
          f'{str(self)}'
      )
...
class CodeVisitor(ast.NodeVisitor):
  """Parses code to collect all required information to produce a `Code`.
  ...
  def return_code(self) -> Code:
    return Code(preface=self._preface, functions=self._functions)


def text_to_code(text: str) -> Code:
  """Returns Code object by parsing input text using Python AST."""
  try:
    # We assume that the code is composed of some preface (e.g. imports,
    # classes, assignments, ...) followed by a sequence of functions.
    tree = ast.parse(text)
    visitor = CodeVisitor(text)
    visitor.visit(tree)
    return visitor.return_code()
...
def text_to_function(text: str) -> Function:
  ...
  if len(code.functions) != 1:
    for f in code.functions:
      if len(f.body) < 6: continue
      else: return f
    raise ValueError(f'Only one function expected, got {len(code.functions)}'
                     f':\n{code.functions}')
  return code.functions[0]
